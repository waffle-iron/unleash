    ...
    return {
      notifyCardOwner: function (data) {
        var messageBody = [
            ...
            '<p>' + data.author.fullName + ' just commented your "' + data.cardType + '" step:</p>',
            '<p>' + data.message + '</p>',
      ...
      notifyCardOwnerReply: function (data) {
        var messagebody = [
          '<h1>Hello, ' + data.cardOwner.fullName + '</h1>',
          '<p>' + data.author.fullName + ' just replied to the comment on your "' + data.cardType + '" step:</p>',
          '<p>' + data.message + '</p>',
          '<p><a href="' + cardUrl(data) + '">Visit the Path</a> to read the full conversation!</p>'
        ].join('');
      ...
      notifyCommentAuthor: function (data) {
        var messageBody = [
          ...
          '<p>' + data.author.fullName + ' just replied to your comment on the "' + data.cardType + '" step:</p>',
          '<p>' + data.message + '</p>',
          '<p><a href="' + cardUrl(data) + '">Visit the Path</a> to read the full conversation!</p>'
        ].join('');
      ...
      notifyReplyAuthor: function (data, previousAuthor) {
        var messageBody = [
          ...
          '<p>' + data.author.fullName + ' just replied to your reply to the comment on the "' + data.cardType + '" step:</p>',
          '<p>' + data.message + '</p>',
          '<p><a href="' + cardUrl(data) + '">Visit the Path</a> to read the full conversation!</p>'
        ].join('');
